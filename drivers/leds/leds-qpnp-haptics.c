		...
		struct device_attribute *attr, char *buf)
{
	...
	rc = qpnp_haptics_read_reg(chip, HAP_VMAX_CFG_REG(chip), &reg, 1);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	reg &= HAP_VMAX_MASK;
	temp = data / HAP_VMAX_MIN_MV;
	reg |= (temp << HAP_VMAX_SHIFT);

	rc = qpnp_haptics_write_reg(chip, HAP_VMAX_CFG_REG(chip), &reg, 1);
