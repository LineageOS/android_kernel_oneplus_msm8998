...
static struct op_adapter_chip *the_chip = NULL;

/*add for dash adapter update*/
#include <linux/power/oem_external_fg.h>

static void dash_uart_gpio_set_value(unsigned long pin, bool value)
{
...
void oem_delay(struct op_adapter_chip *chip,cycles_t begin_time)
{
	...
	if(chip->timer_delay == 52) {
		   ...
		   }while(time < 985);/*990,51us:979,50.5us:970*/
	}
	else if(chip->timer_delay == 78) {
		   do{
			curl_time = get_cycles();
			time = curl_time - begin_time;
		   }while(time < (1468));//1470,76us:1459
	}
...
static void dash_uart_tx_bit(struct op_adapter_chip *chip,	unsigned char tx_data)
{
	...
	switch (tx_bit) {
	    case BIT_START:
	        ...
	        dash_uart_gpio_set_value(chip->uart_tx_gpio, 0);
	        tx_bit = BIT_0;
	        break;
	  	case BIT_0:
	    case BIT_1:
	    ...
	    case BIT_7:
	        if (tx_data & (1 << tx_bit)) {
	            dash_uart_gpio_set_value(chip->uart_tx_gpio, 1);
	        	}
	        else {
	            dash_uart_gpio_set_value(chip->uart_tx_gpio, 0);
	        }
...
static void dash_uart_tx_byte(struct op_adapter_chip *chip, unsigned char tx_data)
{
	cycles_t time;
...
static unsigned char dash_uart_rx_byte(struct op_adapter_chip *chip, unsigned int cmd)
{
...
static int dash_uart_write_some_addr(struct op_adapter_chip *chip, u8 *fw_buf, int length)
{
	...
	while(1) {
		...
		for(i = 0;i < 16;i++) {
			...
			fw_count++;
			if(i == 15) {
				rx_val = dash_uart_rx_byte(chip, Write_Addr_Line_Cmd);
			}
			/*dash_uart_irq_fiq_enable(true);*/
		}
		dash_uart_irq_fiq_enable(true);
...
static bool dash_uart_read_addr_line_and_check(struct op_adapter_chip *chip, unsigned int addr)
{
	...
	for(i = 0; i < 16;i++) {
		fw_check_buf[i + 2] = dash_uart_rx_byte(chip, Read_Addr_Line_Cmd);
		if(chip->rx_timeout)
			goto  read_addr_line_err;
	}
	if(!(addr % 0x20)) {
...
read_addr_line_err:
	...
	if(addr_check_err) {
		...
		pr_err("%s fw_check err,addr:0x%x,check_buf[%d]:0x%x != fw_data[%d]:0x%x\n",
			__func__, addr, i + 2, fw_check_buf[i + 2], (fw_line * 34 + 2 + i),
...
static bool
dash_adapter_update_handle(struct op_adapter_chip *chip, unsigned long tx_pin, unsigned long rx_pin)
{
	unsigned char rx_val = 0;
...
//Step2: Tx_Read_Addr_Line
	...
	for(rx_last_line_count = 0; rx_last_line_count < 18;rx_last_line_count++) {
		rx_last_line[rx_last_line_count] = dash_uart_rx_byte(chip, Read_Addr_Line_Cmd);
		if(chip->rx_timeout)
			break;
	}
	dash_uart_irq_fiq_enable(true);
...
static int __init dash_adapter_init(void)
{
	...
	if(!chip) {
		pr_err("%s dash_adapter alloc fail\n", __func__);
		return -1;
	}
	chip->timer_delay = 0;
	chip->tx_byte_over = false;
